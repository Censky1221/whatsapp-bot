        ...
        switch(command) {
         ...
         break
          ...
          case 'list': case 'store':{
            ...
            //if (!isAlreadyResponListGroup((m.isGroup ? m.chat: botNumber), db_respon_list)) return 
            ...
            break
            case 'dellist':{
                if (!isAdmins) return m.reply('Fitur Khusus admin dan Owner!')
                if (!m.isGroup) return m.reply('Fitur Khusus Group!')
            if (db_respon_list.length === 0) return m.reply(`Belum ada list message di database`)
