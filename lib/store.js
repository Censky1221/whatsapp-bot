...
const GroupList = require(path.join(__dirname, "../database/groupListModel")); 
...
const SewaBot = require("../database/sewaModel");

async function addSewaGroup(gid, expired) {
    const tanggalExpired = new Date(Date.now() + toMs(expired));

    const sewaBaru = new SewaBot({
        id: gid,
    ...
    await sewaBaru.save();
    console.log(`✅ Grup ${gid} berhasil disewa hingga ${tanggalExpired}`);
}

async function getSewaPosition(gid) {
    const sewa = await SewaBot.findOne({ id: gid });
    return sewa ? true : false;
}

async function getSewaExpired(gid) {
    const sewa = await SewaBot.findOne({ id: gid });
...
function getTextSetBot(groupID, _db) {
    ...
    let position = null; // Tambahkan ini sebelum digunakan
    if (position !== null) {
        return _db[position].text
    }

   // Fungsi untuk menambahkan item ke daftar grup
async function addItemToList(groupID, item) {
    try {
        let group = await GroupList.findOne({ groupID });
        if (!group) {
            group = new GroupList({ groupID, list: [] });
        }
...
// Fungsi untuk mendapatkan daftar item dalam grup
async function getList(groupID) {
    try {
        const group = await GroupList.findOne({ groupID });
...
// Fungsi untuk menghapus item dari daftar grup
async function removeItemFromList(groupID, item) {
    try {
        const group = await GroupList.findOne({ groupID });
        if (group) {
            group.list = group.list.filter(i => i !== item);
            await group.save();
            return group;
        }
        return null;
